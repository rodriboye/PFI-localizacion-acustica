...
Modo 'evento' (log_single):
    Fila inmediata con la primera estimación válida — el ángulo del apuntado.
    duration_s = 0 y el rango colapsa a ese ángulo.

Modo 'seguimiento' y sin servo (track + end_event):
    track() acumula las estimaciones del evento en curso sin escribir; al volver
    al silencio, end_event() escribe una fila con el RANGO de ángulos ocupado,
    la duración y los máximos de confianza y energía. close() cierra un evento a
    medio registrar (Ctrl+C durante un seguimiento) para no perderlo.

Columnas:
    timestamp                   inicio del evento (ISO-8601 local)
    mode                        'evento' | 'seguimiento'
    duration_s                  duración del registro (0.0 en modo evento)
    az_deg, el_deg              primera estimación válida
    az_min_deg … el_max_deg     rango ocupado durante el evento
    conf_db, energy_db          máximos del evento (dB)
"""
...
class EventLogger:
    ...
    def __init__(self, filepath):
        ...
        self._file    = None
        self._writer  = None
        self._tracking = None   # evento en curso (modo seguimiento)
        self._open()

    def _open(self):
        # La cabecera solo se escribe si el archivo no existía; si no, se anexa.
    ...
    def track(self, doa_result, energy):
        ...
        No escribe nada: la fila sale en end_event()."""
        if not doa_result or not doa_result.valid:
            return
        az, el = doa_result.azimuth, doa_result.elevation
    ...
    def end_event(self):
        """Cierra el evento en curso y escribe su única fila. Idempotente."""

...
DEFAULT_CONFIG = {
    ...
    'bandpass_low_hz': 200.0,      # corte inferior del pasabanda aplicado al cross-spectrum PHAT (Hz)
    'bandpass_high_margin': 0.9,   # si no se pasa --bandpass-high, se usa este margen * f_max espacial (c / (2*d))
}
...
def gcc_phat(x1, x2, max_delay_samples, freq_mask=None):
    n = len(x1)
    nfft = 2*n
    ...
    eps = 1e-10
    phat = cross_spectrum / (magnitude + eps)

    # pasabanda opcional: ver make_bandpass_mask() mas abajo para el motivo
...
def make_bandpass_mask(n, fs, f_low, f_high, taper_frac=0.1):
    """
    Máscara de pasabanda en frecuencia, para aplicar sobre el cross-spectrum
    PHAT antes de acumularlo/resolverlo.

    Por qué hace falta: PHAT normaliza cada bin de frecuencia a magnitud 1,
    dando el mismo peso a un bin con señal real que a uno con solo ruido.
    Sin este filtro:
      - bins por debajo de f_low (viento, ruido estructural, deriva de DC)
        meten fase aleatoria en la correlación.
      - bins por encima de f_high (más allá de f_max = c / (2*d), ver
        print_system_info) tienen fase ambigua por aliasing espacial y
        pueden generar picos espurios dentro de la ventana de búsqueda.

    Se usa un roll-off coseno (tipo Tukey) en los bordes en vez de un corte
    abrupto, para no introducir ringing (Gibbs) en la función de correlación.

    devuelve un array de longitud n+1 (mismo largo que la salida de
    np.fft.rfft con nfft=2n), con valores en [0, 1].
    """
    nfft = 2 * n
    freqs = np.fft.rfftfreq(nfft, d=1.0 / fs)
    mask = np.ones_like(freqs)

    mask[freqs < f_low] = 0.0
    mask[freqs > f_high] = 0.0

    band = f_high - f_low
    taper_width = max(band * taper_frac, 1e-9)

    rise = (freqs >= f_low) & (freqs < f_low + taper_width)
    mask[rise] = 0.5 * (1 - np.cos(np.pi * (freqs[rise] - f_low) / taper_width))

    fall = (freqs > f_high - taper_width) & (freqs <= f_high)
    mask[fall] = 0.5 * (1 + np.cos(np.pi * (freqs[fall] - (f_high - taper_width)) / taper_width))
...
def parse_args():
    ...
    p.add_argument("--bandpass-low", type=float,
                   default=DEFAULT_CONFIG["bandpass_low_hz"],
                   help=f"Corte inferior del pasabanda antes de GCC-PHAT, en Hz "
                        f"(default: {DEFAULT_CONFIG['bandpass_low_hz']:.0f}). "
                        f"Filtra ruido de baja frecuencia (viento, vibración estructural).")
    p.add_argument("--bandpass-high", type=float, default=None,
                   help="Corte superior del pasabanda, en Hz (default: "
                        f"{DEFAULT_CONFIG['bandpass_high_margin']*100:.0f}%% de f_max espacial "
                        "= margen * c/(2*mic_distance), recalculado según --mic-distance). "
                        "Evita que bins con aliasing espacial degraden el pico de correlación.")
...
def print_system_info(args):
    ...
    angular_res_approx = 180.0 / (2 * tau_max_samples)  # Estimación gruesa, válida cerca de 90° (broadside)

    band_low = args.bandpass_low
    band_high = (args.bandpass_high if args.bandpass_high is not None
                 else DEFAULT_CONFIG["bandpass_high_margin"] * f_max_spatial)
    ...
    print(f"  Pasabanda GCC-PHAT:    {band_low:.0f}–{band_high:.0f} Hz"
          f"{' (¡low >= high, desactivado!)' if band_low >= band_high else ''}")
    print(f"  Resolución angular:    ~{angular_res_approx:.1f}° cerca de 90° (broadside), "
          f"se degrada ~1/sin(θ) hacia 0°/180° (endfire)")
...
def main():
    ...
    # pasabanda sobre el cross-spectrum PHAT (ver make_bandpass_mask para el motivo)
    f_max_spatial = c / (2 * d)
    band_low = args.bandpass_low
    band_high = (args.bandpass_high if args.bandpass_high is not None
                 else DEFAULT_CONFIG["bandpass_high_margin"] * f_max_spatial)
    if band_low >= band_high:
        print(f"[AVISO] --bandpass-low ({band_low:.0f} Hz) >= banda superior "
              f"({band_high:.0f} Hz); pasabanda desactivado.")
        freq_mask = None
    else:
        freq_mask = make_bandpass_mask(args.block_size, fs, band_low, band_high)
    ...
    try:
        while running[0]:
            ...
            phat = gcc_phat(ch_left, ch_right, max_delay_samples, freq_mask=freq_mask)
            if phat_acc is None:
                phat_acc = phat.copy()
            else:
                phat_acc += phat
